...
import { Users, Heart, MessageCircle, Calendar, ShieldCheck, ShieldX, Eye, Trash2, Clock, CheckCircle, XCircle, BarChart3, RefreshCw } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const approuver = async (id) => {
    try {
      await axios.put(`/api/admin/users/${id}/approuver`)
  ...
  const rejeter = async (id) => {
    try {
      await axios.put(`/api/admin/users/${id}/rejeter`)
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-start justify-between fade-up">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Gestion de la plateforme MindCampus</p>
        </div>
      ...
      {/* Tabs */}
      <div className="flex gap-2 bg-gray-100 p-1 rounded-2xl fade-up d1 w-fit">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${tab===t.id ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={r.label} className="text-center p-4 rounded-2xl" style={{background:'#f8fafc',border:'1px solid #f0f0f0'}}>
          ...
          {stats.enAttente > 0 && (
            <div className="p-4 rounded-2xl flex items-center gap-4" style={{background:'linear-gradient(135deg,#fffbeb,#fef3c7)',border:'1px solid #fde68a'}}>
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-600 mt-0.5">Des pair-aidants ou professionnels attendent votre validation</p>
              </div>
              <button onClick={() => setTab('verifications')} className="btn-sm font-semibold px-4 py-2 rounded-xl text-amber-700" style={{background:'#fde68a'}}>
                Voir →
              </button>
            </div>
          )}
            ...
            <div key={u._id} className="card fade-up" style={{animationDelay:`${i*0.05}s`,borderLeft:'3px solid #f59e0b'}}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  <button onClick={() => voirCarte(u._id)}
                  ...
                  <button onClick={() => approuver(u._id)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors">
                    <ShieldCheck className="w-3.5 h-3.5"/> Approuver
                  </button>
                  <button onClick={() => rejeter(u._id)}
      ...
      {tab === 'users' && (
        <div className="space-y-2 fade-up">
          ...
          {users.map((u, i) => (
            <div key={u._id} className="card flex items-center gap-4 py-3 fade-up" style={{animationDelay:`${i*0.03}s`}}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg shrink-0" style={{background:'linear-gradient(135deg,#e0e7ff,#c7d2fe)'}}>
                {roleEmoji[u.role]}
              </div>
              <div className="flex-1 min-w-0">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(0,0,0,0.7)'}}>
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl">
            ...
            {carteModal.carteIdentite ? (
              ...
              <div className="h-40 flex items-center justify-center text-gray-400 bg-gray-50 rounded-2xl">
                Aucune carte soumise
              </div>
            )}
            <div className="flex gap-3 mt-4">
              <button onClick={() => { approuver(carteModal._id); setCarteModal(null) }}
                className="btn-primary flex-1 py-2.5">
                <ShieldCheck className="w-4 h-4"/> Approuver
              </button>
              <button onClick={() => { rejeter(carteModal._id); setCarteModal(null) }}
